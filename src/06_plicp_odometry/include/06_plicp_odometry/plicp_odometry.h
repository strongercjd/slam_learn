...
private:
    ...
    ros::Subscriber odom_sub;        //接收odometry的订阅者对象
    ros::Publisher odom_path_pub;    //发布odom_path的发布者对象
    nav_msgs::Path odom_path;        //发布odom_path的话题
    bool odom_path_init_flag=1;      //path初始化标记位

    ros::Subscriber odom_plicp_sub;        //接收dom_plicp的订阅者对象
    ros::Publisher odom_plicp_path_pub;    //发布dom_plicp_path的发布者对象
    nav_msgs::Path odom_plicp_path;        //发布dom_plicp_path的话题
    bool odom_plicp_path_init_flag=1;      //dom_plicp_path初始化标记位
...
public:
    ...
    void odom_plicpCallback(const nav_msgs::Odometry::ConstPtr& odom);
};
