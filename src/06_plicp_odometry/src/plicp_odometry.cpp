...
#include "06_plicp_odometry/plicp_odometry.h"
...
ScanMatchPLICP::ScanMatchPLICP() : private_node_("~"), tf_listener_(tfBuffer_)
{
    ...
    odom_sub  = node_handle_.subscribe("/odom", 10, &ScanMatchPLICP::odomCallback, this);//订阅odom话题
    odom_path_pub = node_handle_.advertise<nav_msgs::Path>("/odom_path",10, true);//发布odom的path
    odom_path_init_flag=1;

    odom_plicp_sub  = node_handle_.subscribe("/odom_plicp", 10, &ScanMatchPLICP::odom_plicpCallback, this);//订阅odom话题
    odom_plicp_path_pub = node_handle_.advertise<nav_msgs::Path>("/odom_plicp_path",10, true);//发布odom的path
    odom_plicp_path_init_flag=1;
...
void ScanMatchPLICP::odomCallback(const nav_msgs::Odometry::ConstPtr& odom)
{
    if(odom_path_init_flag){
        odom_path.header=odom->header;
        odom_path_init_flag=0;
    }
    ...
    odom_path.poses.push_back(pose_stamped_this);
    odom_path_pub.publish(odom_path);
}

void ScanMatchPLICP::odom_plicpCallback(const nav_msgs::Odometry::ConstPtr& odom)
{
    if(odom_plicp_path_init_flag){
        odom_plicp_path.header=odom->header;
        odom_plicp_path_init_flag=0;
    }
    ...
    odom_plicp_path.poses.push_back(pose_stamped_this);
    odom_plicp_path_pub.publish(odom_plicp_path);
}
